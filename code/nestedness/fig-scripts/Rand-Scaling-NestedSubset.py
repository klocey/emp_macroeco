...
sys.path.append(mydir + "GitHub/emp_nested/code/nestedness/nodf")
import nodf
sys.path.append(mydir + "GitHub/emp_nested/code/nestedness/SiteByTaxa")
import SiteByTaxa
...
for i in range(10000):
    ...
    for j, plevel in enumerate(plevels):
        for k, tlevel in enumerate(tlevels):
            ...
            subsets = []
            if by == 'rows':
                subsets = (10.0**np.linspace(np.log10(20), np.log10(rows-1), num=20, endpoint=True, dtype=np.dtype('f8'))).astype(int).tolist()
            elif by == 'cols':
                subsets = (10.0**np.linspace(np.log10(50), np.log10(cols-100), num=20, endpoint=True, dtype=np.dtype('f8'))).astype(int).tolist()
